...
const SUNRIN_OFFICE = {
  ...
  mapUrls: {
    naver: 'https://map.naver.com/v5/entry/place/12345678', // 사장님 네이버 지도 연동 플레이스 주소
    kakao: 'https://map.kakao.com/12345678'
  }
} as const;

// [AESTHETIC GOVERNANCE] 실용적 미학 선행주의 기반 핵심 톤 체계
const THEME_TOKENS = {
...
export default function SunrinInfrastructureEngine() {
    ...
    switch (activeTrack) {
      ...
      default:
        return {
          ...
          body: "선린은 매물을 포장하여 가볍게 노출하지 않습니다. 계약 전 단계에서 도로명과 지번 주소를 명확히 파악하고, 눈에 보이지 않는 조세 채권 우선순위와 공부상의 리스크를 팩트 그대로 교차 확인해 드리는 동네의 무겁고 조용한 계약 필터입니다."
        };
    }
  }, [activeTrack]);
  ...
  return (
    <div className={`min-h-[100svh] w-full relative flex flex-col justify-between ${THEME_TOKENS.bg} ${THEME_TOKENS.text} overflow-x-hidden select-text font-serif`}>
      ...
      {/* [2] ACCESSIBLE MANILA INDEX DRAWER - 삼선 클릭 시 서랍 개폐 */}
      <nav 
      ...
      {/* [3] CORE IA CONTAINER - 원장 기반 레이아웃 본문 */}
      <main className="max-w-md mx-auto w-full z-10 px-5 py-6 space-y-6 pb-36">
        ...
        <footer className="pt-4 border-t border-[#1B1D1C]/10 space-y-1 text-[0.68rem] font-sans text-gray-400 font-light leading-relaxed text-left">
          ...
          <p className="pt-2 text-[0.62rem] opacity-75 border-t border-[#1B1D1C]/10 break-keep">
            * 정규 내근 업무 시간({SUNRIN_OFFICE.hours.regular}) 외 아침·저녁 대기 시간({SUNRIN_OFFICE.hours.extended})에는 원활한 실무 대조를 위하여 유선을 통한 지번 주소 사전 포착 의뢰만 접수받습니다. 본 시스템은 표시광고법을 엄격히 준수합니다.
          </p>
        </footer>
      </main>

      {/* [4] FIXED CALL BAR - 최하단 고정전화 바 */}
      <div className="fixed bottom-0 left-0 w-full z-50 bg-[#1B1D1C] border-t border-white/10 shadow-[0_-8px_30px_rgba(0,0,0,0.2)] font-sans">
        <div className="flex justify-between items-center px-5 pt-3.5 pb-[max(14px,env(safe-area-inset-bottom))] max-w-md mx-auto">
          ...
          <a
            href={SUNRIN_OFFICE.telHref}
            className="bg-[#8E3224] text-white font-bold px-5 py-3.5 text-[0.88rem] tracking-wide active:brightness-[0.85] transition-all text-center rounded-none"
          >
            전화 상담하기
          </a>
        </div>
