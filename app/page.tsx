...
export default function BenefitLogicPage() {
  ...
  // 대화형 시뮬레이션 기본 초깃값 설정
  const [deposit, setDeposit] = useState<number>(5000); // 보증금 5,000만 원 기본설정
  const [otherLoans, setOtherLoans] = useState<number>(1000); // 선순위 채무 1,000만 원 기본설정

  // 대구광역시 광역시 기본재산액 공제한도: 8,000만 원 고시락킹
  ...
  const estimatedMarketValue = 12000; // 모의 건물 1호 가치 1억 2,000만 원 가정
